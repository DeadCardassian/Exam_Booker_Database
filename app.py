...
def login():
    if request.method == 'POST':
        ...
        cursor.execute("""
            SELECT user_email, user_password_h, user_type
            FROM user
            WHERE user_email = %s
        """, (email,))
...
def register():
    if request.method == 'POST':
        ...
        # TODO: Redirect based on user type, let the user fill more info regrading their user type
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            for row in csv_reader:
                date, start, end, duration, capacity = row

                cursor.execute("""
                    INSERT INTO test_center_availability 
                    (date, start_time, end_time, duration_minutes, capacity)
                    VALUES (%s, %s, %s, %s, %s)
                """, (date, start, end, duration, capacity))
...
def view_availabilities():
    conn = get_connection()
    cursor = conn.cursor(dictionary=True)
    # TODO: this just showed everything we have in TC avail table for now
    cursor.execute("""
        SELECT 
